...
def draw_line(dx, dy, size=50):
    ...
    cv2.line(p, (int(m - l * math.cos(a)), int(m - l * math.sin(a))),
             (int(m + l * math.cos(a)), int(m + l * math.sin(a))), (0, 0, 0), 1)
    s = -1 if dx < 0 else 1
    ...
    def image(self, Q=None):
        ...
        for x in range(self.width):
            for y in range(self.height):
                if (x, y) == self.human:
                    ov = self.pics['human']
                    img[self.size * y + 2:self.size * y + ov.shape[0] + 2,
                    self.size * x + 2:self.size * x + 2 + ov.shape[1], :] = np.minimum(ov, 1.0)
                    continue
                ...
                if self.matrix[x, y] == Board.Cell.empty and Q is not None:
                    ...
                    for i, (ddx, ddy) in enumerate([(-1, 0), (1, 0), (0, -1), (0, 1)]):
                        ...
                        img[self.size * y + 2:self.size * y + l.shape[0] + 2,
                        self.size * x + 2:self.size * x + 2 + l.shape[1], :] = l
